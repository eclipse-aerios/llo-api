...
WORKING := @printf "$(CYAN)🔨 $(RESET)"
DEBUG := @printf "$(MAGENTA)🔍 $(RESET)"
ROCKET := @printf "$(GREEN)🚀 $(RESET)"
PACKAGE := @printf "$(CYAN)📦 $(RESET)"
TRASH := @printf "$(YELLOW)🗑️  $(RESET)"
...
.PHONY: docker-buildx-gitlab
docker-buildx-gitlab: 
	- $(CONTAINER_TOOL) buildx create --name project-v3-builder
	$(CONTAINER_TOOL) buildx use project-v3-builder
	- $(CONTAINER_TOOL) buildx build --provenance=false --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile .
